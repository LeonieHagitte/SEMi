% welche werte für die simulation passen
%(lambdas ausrechnen)
% intercepts unmodeled auf o
% rauschen (reliabilität der moderation
%- R^2 metrik - tabelle?
% reliabilitäten 0.9, 0.7 raus
% Bewertung über some kind of Chi-quadrat metrik
% numerische approximation der funktionsintegrale? Als metrik
% model evaluation - maße für jeden parameter - mi parameter predominantly
...
Neither of the authors has conducted simulation studies that are of immediate relevance to the current project before. However, Andreas Brandmaier was involved in conducting simulation studies on SEM trees/ SEM forests as well as SEM modeling in general \parencite[e.g.][]{Buchberger2024, SilvaDíaz2025, Arnold2021}. 
...
\[
\mathbf{x} = \boldsymbol{\nu}(Z) + \boldsymbol{\Lambda}_x(Z) \cdot \eta + \boldsymbol{\varepsilon}(Z)
\]
...
\begin{itemize}
  ...
  \item $\boldsymbol{\Lambda}_x(Z)$ is the $4 \times 1$ factor loading matrix,
  \item $\boldsymbol{\varepsilon}(Z) \sim \mathcal{N}(\mathbf{0}, \boldsymbol{\Theta}_\varepsilon(Z))$ is the residual vector,
  \item $\eta \sim \mathcal{N}(\mu_\eta(Z), \sigma^2_\eta(Z))$ is the latent factor,
  \item \( Z = h(M) \) is the effective moderator entering the model equations, obtained by transforming a raw continuous covariate \( M \).
\end{itemize}

\vspace{\baselineskip}
\subsubsubsection{\textbf{Moderator Transformation}}

Let \( M \sim \mathcal{N}(0,1) \) denote a raw continuous covariate. The effective moderator is defined as
\[
Z = h(M),
\]
...
\begin{enumerate}
    ...
    h(M) = M.
    \]
    In this case, the effective moderator is unbounded and symmetric around zero.

    \item \textbf{Sigmoid:}
    \[
    h(M) = 2 \cdot \mathrm{logit}^{-1}(\text{slope} \cdot M) - 1,
    \]
    with a fixed slope parameter (e.g., \texttt{sigmoid\_slope\_1} in the implementation). This transformation maps the real line to the open interval \( (-1, 1) \) and yields a bounded moderator symmetric around zero.

    \item \textbf{Quadratic:}
    \[
    h(M) = M^2.
    \]
    Here, the effective moderator takes values on \([0, \infty)\) and is therefore non-negative and right-skewed, capturing nonlinear but strictly non-negative moderation patterns.

    \item \textbf{Noise (non-informative):}
    \[
    h(M) = 0.
    \]
...
\subsubsubsection{\textbf{Factor Loadings}}

The baseline factor loadings were fixed to a discrete value of $\lambda_{xi} = 0.7$ for all items exept item 4, which was used for scaling.  
...
At the population level, the baseline (unmoderated) measurement model is specified with a single latent factor and four indicators. The factor loading matrix is constructed such that the first three indicators share a common loading value $\lambda$, whereas the fourth indicator is fixed to 1.0 to set the scale of the latent factor:
...
The latent factor variance is fixed at a prespecified value $\psi_{\eta} = \psi_\text{var}$, and baseline residual variances are derived analytically from a single target reliability level applied uniformly across all indicators. For each item,
\[
\theta_{i} = 
\frac{\lambda_i^2 \, \psi_{\eta}}{\text{Reliability}} 
\;-\;
\lambda_i^2 \, \psi_{\eta},
\]
yielding a diagonal residual covariance matrix
\[
\boldsymbol{\Theta}_{\varepsilon} = 
\operatorname{diag}(\theta_1,\theta_2,\theta_3,\theta_4).
\]

This baseline specification sets the measurement scale (via the reference loading), ensures positive residual variances for all indicators, and serves as the starting point upon which moderation effects are introduced in subsequent conditions.


For each condition involving moderation, the loading of item $i$ is defined as a deterministic function of the moderator $Z$:

\[
\lambda_{xi}(Z) = \lambda_{xi} + \delta_{\lambda_i} Z,
\quad Z \in [-1, 1].
\]

The parameter $\delta_{\lambda_i}$ represents the strength of the moderation effect and is specified using a discrete set of admissible values chosen to ensure that the moderated loading remains within a psychometrically plausible interval:
...
\[
\delta_{\lambda_i} \in [-0.4,\, 0.3].
\]

To comply with the grid-search framework, $\delta_{\lambda_i}$ is not sampled but instead evaluated at the following discrete levels:

\[
\delta_{\lambda_i} \in \{-0.4,\,-0.2,\,0,\,0.2,\,0.3\}.
\]

These values span the full admissible range while maintaining deterministic, reproducible simulation conditions.
...
Here, $\lambda_1(Z)$ and $\lambda_2(Z)$ are either moderated or unmoderated depending on the corresponding grid condition, whereas $\lambda_3$ and $\lambda_4$ remain fixed across all conditions. The structure of $\boldsymbol{\Lambda}_x(Z)$ is therefore determined jointly by the moderation-design factors and the functional form of $Z$.

\vspace{\baselineskip}

Moderation effects are applied at the case level by transforming an individual-level raw moderator $M$ into an effective moderator $Z = h(M)$ according to the transformation functions described above. For each generated observation, the raw moderator is drawn independently as
\[
M \sim \mathcal{N}(0,1),
\]
and the effective moderator is computed as $Z = h(M)$.

The resulting distributions of $Z$ differ systematically across transformation types:
\begin{itemize}
    \item \textbf{Linear:} $Z = M$, yielding an unbounded moderator symmetric around zero.
    \item \textbf{Sigmoid:} $Z = 2\cdot \mathrm{logit}^{-1}(\text{slope}\cdot M) - 1$, yielding a bounded moderator in $(-1,1)$.
    \item \textbf{Quadratic:} $Z = M^2$, yielding a non-negative moderator with right-skewed distribution concentrated near zero.
    \item \textbf{Noise:} $Z = 0$, yielding a non-informative moderator.
\end{itemize}

For each observation, the effective moderator $Z$ is then inserted into the parameter functions of the measurement model. Two moderation conditions are implemented:

\begin{enumerate}
    \item \textbf{Full moderation (Model 1.1):}  
    All factor loadings and all residual variances vary with $Z$:
    \[
    \lambda_{xi}(Z) = \lambda_{xi} + \delta_{\lambda} Z, 
    \qquad
    \theta_i(Z) = \theta_{i0} \, \bigl(1 + \delta_{\theta} Z\bigr),
    \]
    where $\theta_{i0}$ denotes the baseline residual variance derived from the reliability grid
    (see below). The multiplicative scaling ensures $\theta_i(Z) > 0$ for all cases, provided
    $1 + \delta_{\theta} Z > 0$.

    \item \textbf{Partial moderation (Model 1.2):}  
    Only the loadings of items $1$ and $2$ vary with the moderator:
    \[
    \lambda_{x1}(Z) = \lambda_{x1} + \delta_{\lambda} Z,
    \qquad
    \lambda_{x2}(Z) = \lambda_{x2} + \delta_{\lambda} Z,
    \]
    whereas all residual variances remain fixed at their baseline values $\theta_{i0}$.
\end{enumerate}

The case-wise parameter matrices $\boldsymbol{\Lambda}_x(Z)$ and $\boldsymbol{\Theta}_\varepsilon(Z)$ are therefore obtained by evaluating these functions for each individual subject. Given the magnitudes of the moderation coefficients used in the design, these transformations keep all loadings and residual variances within admissible psychometric ranges across all values of $Z$, ensuring numerically stable and well-defined population covariance structures.

\vspace{\baselineskip}

\textbf{Residual Variances}

Item reliabilities were varied deterministically across simulation conditions using the fixed grid  
\[
\text{Reliability}_i \in \{0.60,\, 0.70,\, 0.80,\, 0.95\}.
\]

For each indicator, a baseline residual variance $\theta_{i0}$ is obtained analytically from the standard reliability expression,
\[
\text{Reliability}_i 
= 
\frac{\lambda_{xi}^2}{\lambda_{xi}^2 + \theta_{i0}}
\quad \Longrightarrow \quad
\theta_{i0}
=
\frac{\lambda_{xi}^2(1-\text{Reliability}_i)}
     {\text{Reliability}_i}.
\]

These baseline values define the unmoderated diagonal residual covariance matrix.

Residual-variance moderation is then introduced multiplicatively as
\[
\theta_i(Z)
=
\theta_{i0}\,\bigl(1 + \delta_{\theta} Z\bigr),
\]
where $\delta_{\theta}$ denotes the residual-variance moderation coefficient.
For items not subject to moderation, $\delta_{\theta}=0$ and thus $\theta_i(Z)=\theta_{i0}$.

This specification combines (a) a reliability-based baseline $\theta_{i0}$ with  
(b) multiplicative moderation, yielding residual variances that remain positive and interpretable across all grid conditions.
...
\nu_i(Z) = \nu_i + \delta_{\nu_i} Z,
\]
where $\nu_i$ denotes the baseline intercept and $\delta_{\nu_i}$ controls the strength and direction of intercept moderation. The pattern of moderated items is varied across three levels:

\begin{enumerate}
    \item \textbf{No intercept moderation:}  
    $\delta_{\nu_i} = 0$ for all $i = 1,\dots,4$, such that
    \[
    \nu_i(Z) = \nu_i \quad \text{for all } i.
    \]

    \item \textbf{Partial intercept moderation (items 1 and 2):}  
    \[
    \delta_{\nu_i} \in \{-1.0,\,-0.5,\,0.0,\,0.5,\,1.0\} \quad \text{for } i \in \{1,2\},
    \qquad
    \delta_{\nu_i} = 0 \quad \text{for } i \in \{3,4\},
    \]
    such that only $\nu_1(Z)$ and $\nu_2(Z)$ vary with $Z$.

    \item \textbf{Full intercept moderation (all items):}  
    \[
    \delta_{\nu_i} \in \{-1.0,\,-0.5,\,0.0,\,0.5,\,1.0\} \quad \text{for } i = 1,\dots,4,
    \]
...
\[
\alpha \in \{0\}, \qquad
\delta_\eta \in \{-1.0,\,-0.5,\,0,\,0.5,\,1.0\}.
\]

Latent variance moderation was modeled using the log-linear form:
\[
\sigma^2_\eta(Z) = \exp(\beta_0 + \beta_1 Z),
\]
where the parameters are varied over a fully specified grid rather than sampled from continuous distributions:
\[
\beta_0 \in \{-0.5,\, 0,\, 0.5\}, \qquad
\beta_1 \in \{-1.0,\,-0.5,\,0,\,0.5,\,1.0\}.
\]


\newpage

\textbf{Analytical Model Study 1}
\begin{center}
    \includegraphics[width=0.7\textwidth]{tikz_model1.png}
\end{center}
\vspace{\baselineskip}
The analytical model will also have one latent factor $\eta_1$. With four manifest indicators ${x_1,..., x_4}$, their factor loadings ${\lambda_1,..., \lambda_4}$ and their respective residuals ${\epsilon_1,..., \epsilon_4}$. In the analytical model the residuals will be assumed and modeled to be uncorrelated. Furthermore, the model includes a moderator variable $Z$, which can moderate the residuals, intercepts and factor loadings from none, one or two of the manifest variables. The moderator variable $Z$ will be modeled as linear or quadratic.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Study 2}
\textbf{Two Factors with Moderator Effects}
We specify a confirmatory factor analysis model with two latent factors, $\eta_1$ and $\eta_2$, and seven observed indicators $\mathbf{x} = (x_1, \dots, x_4)^\top$ and $\mathbf{y} = (y_1, \dots, y_3)^\top$. Along their respective residuals ${\epsilon_1,...,\epsilon_4}$ and ${\delta_1,..., \delta_3}$. The first factor loads onto indicators $x_1$ to $x_4$, the second onto $y_1$ to $y_3$. A continuous moderator variable \( Z \) introduces conditional heterogeneity into selected measurement parameters (i.e. residuals, intercepts, loadings and latent factor covariance) using one of three functional forms (linear, quadratic or sigmoid).
...
\begin{itemize}
  ...
  \item $\boldsymbol{\Lambda}_x(Z)$ is the $4 \times 1$ loading vector for $\eta_1$, and $\boldsymbol{\Lambda}_y(Z)$ is the $3 \times 1$ loading vector for $\eta_2$,
  \item $\boldsymbol{\varepsilon}(Z) \sim \mathcal{N}\bigl(\mathbf{0}, \boldsymbol{\Theta}_x(Z)\bigr)$ and $\boldsymbol{\delta}(Z) \sim \mathcal{N}\bigl(\mathbf{0}, \boldsymbol{\Theta}_y(Z)\bigr)$ are the residual vectors for $\mathbf{x}$ and $\mathbf{y}$,
  \item $\boldsymbol{\eta} = (\eta_1, \eta_2)^\top \sim \mathcal{N}\bigl(\boldsymbol{\mu}_\eta(Z), \boldsymbol{\Phi}_\eta(Z)\bigr)$ is the latent factor vector,
  \item $Z = h(M)$ is the effective moderator entering the model equations, obtained by transforming a raw continuous covariate $M$.
\end{itemize}

\vspace{\baselineskip}
\textbf{Moderator Transformation}
...
\begin{enumerate}
    ...
    \item \textbf{Sigmoid:}
    \[
    h(M) = 2 \cdot \mathrm{logit}^{-1}(\delta M) - 1,
    \]
...
\textbf{Factor Loadings}

For each latent factor, a single reference indicator loading is fixed to 1.0 to set
the measurement scale, and all remaining baseline loadings for that factor are
...
\delta_{\lambda_i} \in [-0.4,\,0.3],
\]
and evaluated on the discrete grid
\[
\delta_{\lambda_i} \in \{-0.4,\,-0.2,\,0,\,0.2,\,0.3\}.
\]
...
\[
\lambda_{x3}(Z)=0.7,\quad
\lambda_{x4}(Z)=1.0,\quad
\lambda_{yj}(Z)=0.7 \quad (j=1,2),\qquad
\lambda_{y3}(Z)=1.0.
\]
...
(iii) moderation strength $\delta_{\lambda_i}$.

\vspace{\baselineskip}
\subsubsubsection{\textbf{Residual Variances}}

Item-specific reliabilities are varied deterministically across simulation
conditions using the fixed grid
\[
\text{Reliability}_i \in \{0.60,\,0.70,\,0.80,\,0.95\}.
\]

Baseline residual variances are derived analytically from the reliability
identity
\[
\text{Reliability}_i 
= 
...
\[
\lambda_{x1}=\lambda_{x2}=\lambda_{x3}=0.7,\qquad
\lambda_{x4}=1.0,
\]
\[
\lambda_{y1}=\lambda_{y2}=0.7,\qquad
\lambda_{y3}=1.0.
\]
These values are used to compute the corresponding baseline residual variances
$\theta_{10},\dots,\theta_{70}$.
...
\theta_{i0}\,\bigl(1+\delta_{\theta} Z\bigr),
\]
where $\delta_{\theta}$ is the residual-variance moderation coefficient.
Indicators that are not subject to residual moderation use $\delta_{\theta}=0$
and therefore retain $\theta_i(Z)=\theta_{i0}$ for all $Z$.

Since residual-variance moderation operates solely through the multiplicative
term above, variation in factor loadings does not affect residual variances.
Indicators with $\delta_{\theta}=0$ therefore retain their baseline values
$\theta_{i0}$ for all $Z$.

\subsubsubsection{\textbf{Latent Variable Distribution}}
...
The parameters governing baseline variance and moderation strength are selected
from discrete grids:
\[
\beta_{0j} \in \{-0.5,\,0,\,0.5\},
\qquad
\beta_{1j} \in \{-0.5,\,0,\,0.5\}.
\]
