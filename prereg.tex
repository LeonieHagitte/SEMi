...
\item
  ...
  not through nonlinear parameter functions. The transformation \(h(M)\)
  is part of the DGM in every moderation condition.
\end{enumerate}

\pandocbounded{\includegraphics[keepaspectratio]{prereg_files/figure-latex/unnamed-chunk-1-1.pdf}}
...
\begin{center}\includegraphics[width=0.7\linewidth]{presentation/tikz_model1} \end{center}
...
will be assumed and modeled to be uncorrelated. For MNLFA, moderation of
selected intercepts and factor loadings is modeled parametrically. In
the main study, two MNLFA specifications will be evaluated: one assuming
linear moderation in the observed moderator(s), and one assuming
quadratic moderation. Therefore, MNLFA is correctly specified only when
the analytical functional form matches the DGM transformation \(h(M)\),
and is otherwise intentionally misspecified. For most population models,
the MNLFA includes \(M_1\) as the focal moderator. In the interaction
population model, an additional MNLFA specification includes \(M_1\),
\(M_2\), and \(M_1 \times M_2\).
...
variable(s) from the candidate set.
...
\textbf{Intercepts:} Intercept baselines are set to 1 across simulation
conditions.\\
...
a Bonferroni correction to account for multiple comparisons at each
...
maximum depth of the tree to 3 and \texttt{min.N} is set to 50. Because
the MNLFA also considers directly the form of the moderation, which the
SEM trees do not, we further want to use SEM forests for generating
partial dependence plots to get an approximation of the functional form
...
\texttt{semtree} (Andreas M. Brandmaier et al. 2026) package using
OpenMx RAM models. Simulation design construction, data handling,
aggregation, and visualization will rely on \texttt{dplyr}(Wickham et
al. 2026), \texttt{tidyr} (Wickham, Vaughan, and Girlich 2025),
\texttt{tibble} (Müller and Wickham 2026), \texttt{purrr} (Wickham and
Henry 2026), \texttt{furrr} (Vaughan and Dancho 2022), \texttt{future}
...
Andreas M. Brandmaier, John J. Prindle, Manuel Arnold, and Caspar J. Van
