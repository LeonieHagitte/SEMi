...
\begin{itemize}
  ...
  \item $\boldsymbol{\varepsilon} \sim \mathcal{N}\left(\mathbf{0}, \boldsymbol{\Theta}_\varepsilon \right)$ is the residual vector,
...
\[
M^\star = \mathrm{clip}(M,-c,c)
=
\begin{cases}
-c, & \text{if } M < -c,\\
M,  & \text{if } -c \le M \le c,\\
c,  & \text{if } M > c,
\end{cases}
\]
...
\begin{enumerate}
    ...
    \item \textbf{Sigmoid:}
    \[
    h(M^\star) = 2 \cdot \mathrm{logit}^{-1}(kM^\star) - 1,
    \]
    with a fixed steepness parameter \(k > 0\).
    ...
    \[
    h(M^\star) = 2 \cdot \mathrm{logit}^{-1}\!\bigl(\lambda \bigl((M^\star)^2 - 1\bigr)\bigr) - 1,
    \]
    with \(\lambda > 0\).
    This transformation captures symmetric nonlinear moderation as a function of distance from the center while preventing unbounded growth of the moderator.
...
All parameter-level moderation effects are specified as linear functions of the effective moderator $Z$. Nonlinearity in moderation arises exclusively through the transformation $Z = h(M^\star)$ of the raw covariate $M$, not through nonlinear parameter functions.
...
$\psi_{\eta}= ##$ and is held constant across all simulation conditions. %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\text{Reliability}_i
=
\frac{\lambda_{xi}^2 \psi_0}{\lambda_{xi}^2 \psi_0 + \theta_{i0}}
\quad \Rightarrow \quad
\theta_{i0}
=
\frac{\lambda_{xi}^2 \psi_0(1-\text{Reliability}_i)}
     {\text{Reliability}_i}.
\]
...
and therefore retain $\theta(Z)=\theta_{0}$ for all $Z$.
