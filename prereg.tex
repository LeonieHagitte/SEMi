...
\begin{itemize}
  ...
  \item $\boldsymbol{\varepsilon}(Z) \sim \mathcal{N}\left(\mathbf{0}, \boldsymbol{\Theta}_\varepsilon(Z)\right)$ is the residual vector,
  \item $\eta \sim \mathcal{N}\left(\mu_\eta(Z), \sigma^2_\eta(Z)\right)$ is the latent factor,
...
At the population level, the baseline (unmoderated) measurement model is specified with a single latent factor and four indicators. The factor loading matrix is constructed such that all fourindicators share a common loading value $\lambda$:
...
This baseline specification ensures positive residual variances for all indicators reflecting a chosen degree of reliability, and serves as the starting point upon which moderation effects are introduced in subsequent conditions.
...
To comply with the grid-search framework, $\delta_{\lambda_i}$ is not sampled but instead evaluated at the following discrete levels (without the "zero" as this would represent no effect):

\[
\delta_{\lambda_i} \in \{-0.4,\,-0.2,\,0.2,\,0.3\}.
\]
