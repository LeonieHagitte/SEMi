...
casp_mnlfamxsem <- '
# Loadings
control  =~ {lC1 := lC0_1 + lC1_1*data.Age + lC2_1*data.Male}*cC1 +
  {lC2 := lC0_2 + lC1_2*data.Age + lC2_2*data.Male}*cC2 +
  {lC3 := lC0_3 + lC1_3*data.Age + lC2_3*data.Male}*cC3

autonomy =~ {lA1 := lA0_1 + lA1_1*data.Age + lA2_1*data.Male}*cA1 +
  {lA2 := lA0_2 + lA1_2*data.Age + lA2_2*data.Male}*cA2 +
  {lA3 := lA0_3 + lA1_3*data.Age + lA2_3*data.Male}*cA3

pleasure =~ {lP1 := lP0_1 + lP1_1*data.Age + lP2_1*data.Male}*cP1 +
  {lP2 := lP0_2 + lP1_2*data.Age + lP2_2*data.Male}*cP2 +
  {lP3 := lP0_3 + lP1_3*data.Age + lP2_3*data.Male}*cP3

self_real =~ {lS1 := lS0_1 + lS1_1*data.Age + lS2_1*data.Male}*cS1 +
  {lS2 := lS0_2 + lS1_2*data.Age + lS2_2*data.Male}*cS2 +
  {lS3 := lS0_3 + lS1_3*data.Age + lS2_3*data.Male}*cS3

# Explanation: Loadings specify the relationships between latent variables and observed indicators.
# Each loading is a function of intercepts, slopes (coefficients), and covariates (Age and Male).
...
# Manifest Variances
cC1  ~~ {vC1 := exp(vC0_1 + vC1_1*data.Age + vC2_1*data.Male)}*cC1
cC2  ~~ {vC2 := exp(vC0_2 + vC1_2*data.Age + vC2_2*data.Male)}*cC2
cC3  ~~ {vC3 := exp(vC0_3 + vC1_3*data.Age + vC2_3*data.Male)}*cC3

cA1  ~~ {vA1 := exp(vA0_1 + vA1_1*data.Age + vA2_1*data.Male)}*cA1
cA2  ~~ {vA2 := exp(vA0_2 + vA1_2*data.Age + vA2_2*data.Male)}*cA2
cA3  ~~ {vA3 := exp(vA0_3 + vA1_3*data.Age + vA2_3*data.Male)}*cA3

cP1  ~~ {vP1 := exp(vP0_1 + vP1_1*data.Age + vP2_1*data.Male)}*cP1
cP2  ~~ {vP2 := exp(vP0_2 + vP1_2*data.Age + vP2_2*data.Male)}*cP2
cP3  ~~ {vP3 := exp(vP0_3 + vP1_3*data.Age + vP2_3*data.Male)}*cP3

cS1  ~~ {vS1 := exp(vS0_1 + vS1_1*data.Age + vS2_1*data.Male)}*cS1
cS2  ~~ {vS2 := exp(vS0_2 + vS1_2*data.Age + vS2_2*data.Male)}*cS2
cS3  ~~ {vS3 := exp(vS0_3 + vS1_3*data.Age + vS2_3*data.Male)}*cS3

# Explanation: Manifest variances are specified as exponential functions of intercepts, slopes, and covariates.

# Intercepts
cC1  ~ {iC1 := iC0_1 + iC1_1*data.Age + iC2_1*data.Male}*1
cC2  ~ {iC2 := iC0_2 + iC1_2*data.Age + iC2_2*data.Male}*1
cC3  ~ {iC3 := iC0_3 + iC1_3*data.Age + iC2_3*data.Male}*1

cA1  ~ {iA1 := iA0_1 + iA1_1*data.Age + iA2_1*data.Male}*1
cA2  ~ {iA2 := iA0_2 + iA1_2*data.Age + iA2_2*data.Male}*1
cA3  ~ {iA3 := iA0_3 + iA1_3*data.Age + iA2_3*data.Male}*1

cP1  ~ {iP1 := iP0_1 + iP1_1*data.Age + iP2_1*data.Male}*1
cP2  ~ {iP2 := iP0_2 + iP1_2*data.Age + iP2_2*data.Male}*1
cP3  ~ {iP3 := iP0_3 + iP1_3*data.Age + iP2_3*data.Male}*1

cS1  ~ {iS1 := iS0_1 + iS1_1*data.Age + iS2_1*data.Male}*1
cS2  ~ {iS2 := iS0_2 + iS1_2*data.Age + iS2_2*data.Male}*1
cS3  ~ {iS3 := iS0_3 + iS1_3*data.Age + iS2_3*data.Male}*1
...
# casp_mnlfamxsem_result <- mxTryHard(casp_mnlfamxsem)
