...
Age as covariate is included continuously, gender is included as dichotomous variable and countries are clustered in four categories.  
All covariates are included simultaneously in the analytical model. 

CASP responses outside the valid range of 1–4 were coded as missing, and positively worded items were reverse-scored so that all items had a common direction. 
Gender was coded as 0 = male and 1 = female. Countries were grouped into Nordic, Western, Southern, and Eastern European cultural clusters,following @BonnevilleRoussy2024, with Western Europe serving as the reference group for MNLFA. 
Invalid negative age values were coded as missing. Participants with missing age, gender, or cultural-cluster information and participants with no observed CASP-12 items were excluded. 
Age was subsequently standardized within the resulting analysis sample. 
For analyses requiring complete CASP data, a complete-case sample containing respondents with observations on all 12 items was additionally constructed.
...
-->
An important qualification to the comparatively robust power of SEM trees concerns their ability to distinguish between different levels of MNI. 
Under complete MI, rejection rates were generally close to the nominal level, suggesting adequate calibration when neither loadings nor intercepts were moderated. 
However, metric rejection increased in several conditions in which loadings were invariant but intercepts were noninvariant. 
This pattern suggests that the metric SEM-tree procedure may be sensitive to parameter instability originating elsewhere in the measurement model, even when the focal loading parameters are invariant. 
The increase in metric rejection with sample size in some of these conditions further suggests that this behavior cannot simply be attributed to sampling variability at smaller sample sizes. 
Instead, greater statistical information may increase the ability of the score-based procedure to detect model instability that is not perfectly localized to the focal parameter set.
...
The high power of SEM trees indicates that they were effective at identifying heterogeneity in the measurement model across the conditions considered here, but a detected split should not necessarily be interpreted as uniquely identifying the parameter class responsible for that heterogeneity. 
This became particularly apparent for scalar-only MNI. Although the scalar SEM-tree analysis showed very high sensitivity to intercept instability, frequent metric rejection reduced the accuracy with which scalar-only MNI was identified under the sequential decision rule. 
Thus, the advantages of SEM trees for detecting previously unspecified forms of heterogeneity need to be considered separately from their ability to classify that heterogeneity as specifically metric or scalar.

Differences in scalar performance between MNLFA and SEM trees should be interpreted cautiously because, although the same sequential decision rule was applied to both approaches, their underlying scalar tests were constructed differently. 
For both methods, metric and scalar tests were estimated in each replication, and scalar MNI was classified sequentially only when the metric test did not reject. 
However, the SEM-tree scalar test was estimated as a separate intercept-focused analysis and was therefore not conditional on the metric-tree partition. 
In MNLFA, by contrast, the scalar LRT compared the metric and scalar models, such that intercept moderation was tested under the restriction that loading moderation was absent. 
Consequently, differences in level-specific scalar power between the methods may partly reflect differences in the construction of the underlying scalar tests rather than differences in sensitivity alone.
