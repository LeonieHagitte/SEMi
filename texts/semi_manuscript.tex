...
we model quality of life assessed by the CASP-12 (Borrat-Besson et al.
2015; Wiggins et al. 2008). The illustration is motivated by the study
from Bonneville-Roussy et al. (2024), who examined longitudinal and
cross-sectional MI of the CASP-12 across age, gender, and cultural
regions using group-based comparisons. Age is a particularly useful case
for comparison: conventional MGCFA, which requires discretizing
continous age into age groups, whereas the approaches considered here
can incorporate age as a continuous covariate, avoiding potentially
...
\bottomrule\noalign{}
\endlastfoot
Population model & L0I0, LAI0, L0IA, LAIA, L2I0, L0I2, L2I2, 1.3,
1.32 \\
...
In the candidate-variable robustness analysis (1,000 replications),
metric non invariance was detected in 79.1\% of replications and the
true moderator was identified in 77.4\%. The true moderator alone was
selected in 68.2\% of replications, while the true moderator and at
least one noise variable were jointly selected in 9.2\%. Selection of
noise variables in the absence of the true moderator occured in 1.7\% of
the cases.

\section{Empirical Example}\label{empirical-example}

For the application we used the data on the CASP-12 scale from Wave 9 of
the Survey of Health, Ageing and Retirement in Europe (SHARE). The
CASP-12 is a shorter version of the CASP-19, a measure to assess quality
of life in old age (Hyde et al. 2003). The CASP-12 comprises 12 items
assessing four domains of quality of life: \emph{Control},
\emph{Autonomy}, \emph{Self-Realization}, and \emph{Pleasure}, with
three items assigned to each domain. Responses are recorded on a
four-point rating scale ranging from 1 (Often) to 4 (Never). The shorter
version was later suggested by (\textbf{Wiggins2008?}) still differs
from the version used in SHARE, by two items. Instead of the items ``I
feel that I can please myself what I do.'', SHARE includes the item
``Wie oft denken Sie, dass familiäre Verpflichtungen Sie daran hindern,
das zu tun, was Sie möchten?'' (A2). Furthermore, instead of the item
``I enjoy the things that I do.'', SHARE includes the item ``Wie oft
blicken Sie - unterm Strich - mit einem Gefühl des Glücks auf Ihr Leben
zurück?'' (P3). In the present analyses, the four-category CASP-12 items
were treated as continuous indicators. Accordingly, scalar measurement
parameters refer to item intercepts. Positively worded items were
reverse-scored so that all items had a common direction, with higher
values indicating better quality of life.

Following the characteristics examined by Bonneville-Roussy et al.
(2024), age, gender, and cultural region were considered as potential
sources of measurement heterogeneity. Age was included as a continuous
covariate, gender as a dichotomous covariate, and countries were grouped
into four cultural regions: Nordic, Western, Southern, and Eastern
Europe. All covariates were considered simultaneously in the analytical
models. Gender was coded as 0 = male and 1 = female, and Western Europe
served as the reference category for cultural region in the MNLFA
analyses.
...
forest (Brandmaier et al. 2016) was additionally estimated to examine
the functional form of moderator-related parameter variation. Partial
dependence plots (PDPs) were used to obtain a flexible, tree-based
...
As a first sanity check we performed a CFA, using WLSMV as estimator, to
see how that model fitted in general. The simpel CFA model for CASP-12
(\(X^2\) = 21508.260; \emph{df} = 48) showed acceptable fit indices with
CFI = .94; TLI = .92; RMSEA = .08 {[}.082-.084{]} and a SRMR of .05. The
loadings onto the items were generally high only the factor
\emph{Autonomy} showed lower loadings onto its items
(\(\lambda_{A1}\)=.58; \(\lambda_{A2}\)=.25; \(\lambda_{A3}\)=.45). The
results can be seen in the Appendix table .
...
function of the included moderators. The estimated moderation
coefficients suggested that MNI was primarily associated with age and
cultural cluster, whereas gender-related differences were comparatively
small (Tables X and X). Relative to the Western European reference
group, the largest loading-moderation coefficients for Southern Europe
were observed for A3 \((\hat\beta=.423)\) and C3 \((\hat\beta=.358)\),
while the corresponding coefficients for Eastern Europe were largest for
A3 \((\hat\beta=.345)\) and A2 \((\hat\beta=.297)\). Gender-related
...
The PDP show the factor loadings, predicted by the SEM forest, across
age while averaging over the remaining predictors. The PDPs were
generated based on 10 trees with a depth of 3 and a case-minimum of 500.

\begin{center}\includegraphics[width=0.8\linewidth]{PDP_plot} \end{center}

The PDPs suggest that some of the inspected loadings might not be
modearted linearly by age. For the items A2 and A3, loadings were rather
stable at the lower age range and decreased with age, the largest
difference occuring between mid-60s and early 70s. For the factor
Pleasure the pattern seems to be more heterogeneous, as the loading of
item P2 seems relatively stable across Age, whereas item P3 shows a more
pronounced decline, also starting in mid 60s.
...
Einfluss darauf zu haben, was mit Ihnen geschieht?'') and A3 (``Wie oft
denken Sie, dass fehlendes Geld Sie davon abhält, die Dinge zu tun, die
Sie tun möchten?'').
...
categorical and continuous background characteristics (Bauer et al.
2020; Belzak and Bauer 2024). Although such extensions can reduce the
need to prespecify individual sources of measurement heterogeneity, they
also introduce additional methodological choices concerning penalization
and model selection. The present study therefore focuses on MNLFA and
SEM trees without regularization.
...
\bibitem[\citeproctext]{ref-Hyde2003}
Hyde, M., R. D. Wiggins, P. Higgs, and D. B. Blane. 2003. {``A Measure
of Quality of Life in Early Old Age: The Theory, Development and
Properties of a Needs Satisfaction Model (CASP-19).''} \emph{Aging \&
Mental Health} 7 (3): 186--94.
\url{https://doi.org/10.1080/1360786031000101157}.
...
{Sajobi, Tolulope T, Olayinka I Arimoro, Nancy E Mayo, et al.} 2026.
