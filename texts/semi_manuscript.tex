...
interpretation. Conventional approaches to MI typically represent
heterogeneity in measurement parameters through predefined groups or
discrete conditions. This representation becomes restrictive when
candidate sources of measurement heterogeneity are continuous, such as
age or other individual characteristics, particularly when the
functional relationships between these covariates and measurement
parameters are unknown. In these settings, discretizing continuous
covariates may obscure the form and location of measurement
non-invariance, whereas imposing a prespecified functional relationship
risks misspecifying the underlying heterogeneity. Thus, a central
methodological problem is how to represent and detect heterogeneity in
measurement parameters across continuous covariates without requiring
either predefined groups or a known functional form.
...
parameter variation can be specified a priori. \#\# Moderated Nonlinear
Factor Analysis Moderated nonlinear factor analysis (MNLFA) meets this
requirement by modeling factor loadings, intercepts, and other
...
continuous moderators of the measurement model. This specification
becomes challenging when there is limited prior knowledge about which
candidate covariates are associated with measurement parameters, which
parameters exhibit heterogeneity, or what functional forms characterize
these relationships. Such settings motivate methods that can explore
parameter heterogeneity without requiring these moderation relationships
to be fully specified a priori.
...
without requiring the dunctional form to be specified parametrically, by
...
take. At the same time, the approach remains closely aligned with
familiar MGCFA practice, making SEM trees a comparatively accessible
extension of conventional MI analysis.
...
partitioning variables, and potentially nonlinear forms of
heterogeneity, given the sample size is large enough to detect such
effects. Although explicit specification of moderators and moderation
functions remains advantageous when strong theoretical expectations are
available, SEM trees provide a complementary strategy for situations in
which the sources or functional forms of heterogeneity are uncertain.

\subsection{Present Study}\label{present-study}

MGCFA, MNLFA, and SEM trees differ in how they represent heterogeneity
in measurement parameters. MGCFA represents heterogeneity through
differences between predefined groups. When heterogeneity is associated
with a continuous covariate \(z\), however, the methodological problem
can instead be expressed as determining how a measurement parameter
\(\theta\) varies as a function of that covariate, \(\theta(z)\). MNLFA
and SEM trees provide fundamentally different representations of this
relationship. MNLFA represents \(\theta(z)\) through parametrically
specified moderation functions. SEM trees, in contrast, represent
variation in \(\theta(z)\) through data-driven recursive partitions of
the covariate space. Thus, the methods differ not simply in flexibility,
but in how they represent the relationship between covariates and
measurement parameters. The present simulation study examines the
consequences of these different representations for detecting MNI.
Specifically, we compare MNLFA and SEM trees performance in detecting
MNI, when the true parameter--covariate relationship is linear,
quadratic, or sigmoid, while additionally varying the magnitude of
parameter heterogeneity and sample size. Rather than assuming that one
representation is generally superior, we examine how their performance
depends on characteristics of the underlying heterogeneity and the
information available to each method.

The comparison addresses four aspects of performance. First, we examine
...
characterized by nonlinear moderation, especially sigmoid relationships,
whereas their relative advantage should diminish in purely linear
settings. In contrast, MNLFA is expected to perform best when the
functional form of moderation is correctly specified, particularly in
linear conditions, but to be more sensitive to misspecification of the
underlying moderation function. For both methods, stronger moderation
effects are expected to increase the detectability of measurement
non-invariance.
...
factor analysis (EFA) trees, and Goretzko et al. (2026) subsequently
extended this work to CFA trees. Like SEM trees more generally, EFA and
CFA trees use model-based recursive partitioning and score-based
structural-change tests to identify parameter instability across
candidate covariates. EFA trees embed an exploratory factor model within
this framework, whereas CFA trees retain a prespecified confirmatory
measurement structure and can be configured to investigate particular
classes of measurement parameters, including item intercepts in analyses
of scalar MNI.

The procedure examined in the present study is likewise a CFA-based
application of the SEM-tree framework. Metric and scalar MNI are
...
the intercept-focused CFA-tree procedure described by Goretzko et al.
(2026), in which nonfocal model parameters can be fixed while intercepts
are evaluated for instability. Rather than proposing a distinct
tree-based MI method, the present study therefore evaluates the
...
In addition to the simulation study, we illustrate the approaches using
the CASP-12 measure of quality of life from the ninth wave of the Survey
of Health, Ageing and Retirement in Europe (SHARE-ERIC 2024).\\
The CASP-12 is a shorter version of the original 19 item version of the
CASP scale. Additionally the shorter CASP-12 version, suggested by
Wiggins et al. (2008) is also slightly different from the version used
in the SHARE assessment (Borrat-Besson et al. 2015; Wiggins et al.
2008). This application is motivated by Bonneville-Roussy et al. (2024),
who investigated MI of the CASP-12 across age, gender, and cultural
regions using group-based comparisons. The application provides an
empirical setting in which potential sources of measurement
heterogeneity include both categorical characteristics and age as a
continuous characteristic, allowing us to examine how approaches that do
not require age to be categorized represent heterogeneity in the CASP-12
measurement model. This highlights the implicit need for categorization,
when using MGCFA to test for MI. We want to use this as a possible
example case to use these methods for MI testing, that allow for the
variables to be included as continuous variables, possibly avoiding
arbitrary categorization. Bonneville-Roussy et al. (2024) investigated
the longitudinal as well as the cross-sectional MI. For this empirical
...
and four sample sizes resulted in 576 data-generating design cells. Each
population condition was evaluated using linear MNLFA, quadratic MNLFA,
and SEM trees, yielding 1,728 method-specific design cells.

The simulation distinguished a primary method comparison from a separate
candidate-selection robustness analysis. In the primary comparison, the
relevant data-generating moderator or moderators were supplied to the
SEM trees as candidate partitioning variables. Thus, the primary
comparison did not require SEM trees to identify the relevant moderator
from a larger set of irrelevant covariates. Instead, it was designed to
compare how the methods represent moderator-related parameter
heterogeneity: MNLFA through prespecified parametric moderator functions
and SEM trees through recursive partitioning of the moderator space.
...
entered the instability test; nonfocused free parameters were not
...
runs, resulting in 1,728 method-specific analysis cells. An additional
100 SEM-tree analyses were conducted for the noisy-predictor sensitivity
condition.
...
examined by Bonneville-Roussy et al. (2024), age, gender, and cultural
region were considered as potential sources of measurement
heterogeneity. Age was included as a continuous covariate, gender as a
dichotomous covariate, and countries were grouped into four cultural
regions: Nordic, Western, Southern, and Eastern Europe. All covariates
were considered simultaneously in the analytical models. Gender was
coded as 0 = male and 1 = female, and Western Europe served as the
reference category for cultural region in the MNLFA analyses.
...
had missing information on age, gender, or cultural region or had no
observed CASP-12 items. Age was standardized within the resulting
analysis sample. The final analysis sample comprised \(N=65{,}215\)
participants.
...
visualization did rely on \texttt{dplyr}(Wickham et al. 2026),
...
Across 172,800 method-specific simulation runs, 42 (0.024\%) did not
yield evaluable metric or scalar test results. This occurred in 11 of
57,600 MNLFA runs (0.019\%) and 31 of 57,600 MNLFAQ runs (0.054\%); all
...
rates were .063 for linear MNLFA, .067 for quadratic MNLFA, and .050 for
SEM trees. Corresponding scalar rejection rates were .052, .053, and
.044, respectively (see table 2). Rejection rates were relatively stable
...
power under linear (.970) and sigmoid (.997) moderation and
substantially lower power under quadratic moderation (.166). The
complementary pattern occurred for quadratic MNLFA, which showed high
power under quadratic moderation (.978) but considerably lower power
under linear (.407) and sigmoid (.258) moderation. SEM tree power was
comparatively stable across functional forms, ranging from .903 to .934.
...
power under quadratic moderation (.089), whereas quadratic MNLFA
achieved complete power under quadratic moderation but little power
under linear (.066) or sigmoid (.054) moderation. SEM tree scalar power
remained at or above .992 across all three functional forms.
...
population models but was comparatively lower for Model 1.32 (.769), in
...
rejection under loading invariance averaged .058 for linear MNLFA,
compared with .453 for quadratic MNLFA and .350 for SEM trees. Thus,
...
\begin{center}\includegraphics[width=0.8\linewidth]{Rplot2} \end{center}
...
metric-only MNI (.959) and combined metric and scalar MNI (.884),
compared with .699 and .723 for linear MNLFA and .418 and .678 for
...
classified scalar-only MNI in only .635 of replications because the
metric test rejected first in .362 of replications. Linear MNLFA showed
similar sequential accuracy for scalar-only MNI (.645), whereas
quadratic MNLFA showed lower accuracy (.332). Consequently, high power
...
The MNLFA provided evidence against both metric and scalar MI of the
CASP-12. Constraining loading-moderation effects to zero resulted in a
significant deterioration in model fit relative to the unrestricted
model \((\Delta\chi^2(40)=2524.64; p<.001)\). Subsequently constraining
intercept-moderation effects to zero resulted in a statistically
significant deterioration in fit,
\((\Delta\chi^2(40)=13541.93; p<.001)\) (Table X). Thus, the MNLFA
indicated moderation of both factor loadings and item intercepts.
...
\((\hat\beta=.345)\) and A2 \((\hat\beta=.297)\). Age-related loading
moderation was smaller in magnitude, with the largest coefficients
observed for A3 \((\hat\beta=-.098)\) and A2 \((\hat\beta=-.084)\).
...
expressed over the observed moderator range. This finding indicates,
that additional statistical information cannot compensate for a
moderator function that inadequately represents the underlying
...
advantage is particularly relevant in applications in which theory
...
should also be considered in relation to Bonneville-Roussy et al.
(2024), whose analysis of the CASP-12 provided the substantive
motivation for the present application. Their results supported the
four-factor structure of the CASP-12 and indicated full gender
invariance, largely age-invariant measurement, and partial scalar
...
older. Whereas Bonneville-Roussy et al. (2024) found cross-sectional age
invariance for 11 of the 12 CASP-12 items, both MNLFA and SEM trees
identified age as a relevant source of parameter heterogeneity in the
present data. This difference should be interpreted in light of the
different analytical representations of age. In the group-based analysis
of Bonneville-Roussy et al. (2024), age differences were evaluated
across predefined age groups(younger than 60 years, 60--69 years, 70--79
years, and 80 years and older), whereas the present analyses retained
age as a continuous covariate. MNLFA therefore allowed measurement
parameters to vary continuously with age, while SEM trees identified
data-driven age thresholds at which parameter instability was most
pronounced. Heterogeneity that is localized within or across predefined
age groups may therefore be more difficult to detect when age is
discretized in advance.

The SEM tree results provide a particularly informative comparison with
their group-based age analysis. Several of the age cutoffs identified
from the data closely corresponded to the boundaries used by
Bonneville-Roussy et al. (2024). In the metric tree, age partitions
...
groups in the analysis from Bonneville-Roussy et al. (2024). This
correspondence provides some empirical support for age-related
distinctions similar to those imposed a priori in the earlier study. The
additional cutoffs identified by the SEM trees illustrate the potential
consequences of defining age groups in advance. The metric split around
65 years suggests measurement heterogeneity within the predefined
60--69-year group, whereas the scalar splits around 87--88 years
indicate further heterogeneity within the broad group of participants
aged 80 years and older. Conversely, the SEM trees did not reproduce
every predefined boundary, most notably the distinction at 60 years. The
MNLFA results complement this pattern by showing that age-related
heterogeneity can also be represented without introducing discrete age
categories. Age significantly moderated both loadings and intercepts,
with comparatively pronounced coefficients for A2 and A3. Together,
these findings help reconcile the present results with the largely
age-invariant findings reported by Bonneville-Roussy et al. (2024). The
difference need not imply contradictory conclusions about the CASP-12.
Instead, group-based MI analyses evaluate whether measurement parameters
differ across the particular age categories specified by the researcher,
whereas MNLFA and SEM trees can reveal more localized patterns of
age-related heterogeneity that may occur within or around those
categories.

There was also some convergence at the item level. Bonneville-Roussy et
al. (2024) identified an item from the Autonomy dimension as showing
age-related MNI, and several of the comparatively larger age-related
moderation coefficients in the present MNLFA were likewise observed for
Autonomy items, particularly A2 and A3. This correspondence suggests
that the Autonomy dimension may warrant particular attention when
evaluating the CASP-12 across age. However, the present results also
implicated items from other domains and should therefore not be
interpreted as indicating that age-related heterogeneity is confined to
Autonomy. More generally, the comparison illustrates how continuous and
data-driven approaches can complement conventional group-based MI
analyses by refining the characterization of heterogeneity that may
appear limited when evaluated only across predefined groups. Also please
note, that we inspected the ninth wave of SHARE, Whereas
Bonneville-Roussy et al. (2024) investigated the waves two and seven.
The empirical example therefore illustrates that MNLFA and SEM trees can
yield broadly convergent substantive conclusions while providing
different descriptions of the underlying measurement heterogeneity.
...
usefulness depends partly on the substantive question and the amount of
...
with .058 for linear MNLFA, despite the absence of loading MNI. The
corresponding pattern in the opposite direction was much weaker: when
loadings were noninvariant but intercepts were invariant, scalar
rejection generally remained close to the nominal level. Thus, the
cross-level sensitivity observed for SEM trees was asymmetric. Intercept
instability could produce evidence against metric invariance, whereas
loading instability did not produce a comparable tendency to reject
scalar invariance. Moreover, metric rejection in these scalar-only
conditions increased rather than decreased with sample size in some
conditions, particularly for SEM trees. This pattern suggests that the
elevated rejection rates reflect systematic sensitivity of the metric
test to instability outside the focal loading parameters rather than
merely imprecision in smaller samples.
...
rates can conceal substantively different sources of error: failure to
detect MNI under functional-form misspecification in MNLFA versus
detection of measurement heterogeneity at the wrong invariance level in
SEM trees.
...
CFA trees (Goretzko et al. (2026)) and are better understood as a
particular specification of the broader SEM-tree framework than as a
distinct class of tree methods. In both cases, a prespecified
confirmatory factor model is embedded within model-based recursive
partitioning. In the present study, however, metric and scalar MNI were
investigated by defining factor loadings or item intercepts,
...
described by Goretzko et al. (2026) for investigating scalar MI. In that
procedure, parameters other than the item intercepts can be fixed to
estimates from a single-group CFA, such that only the intercepts are
freely estimated and evaluated for splitting. The distinction is
therefore not between CFA trees and SEM trees, as used in this study, as
fundamentally different algorithms, but between different ways of
constraining and focusing the SEM-tree model. The present approach uses
parameter-focused score tests within an otherwise freely estimated CFA
model, whereas the scalar CFA tree procedure described by Goretzko et
al. (2026) more strictly isolates intercept instability by fixing all
remaining model parameters.

This distinction is relevant to the present simulation results. Although
focusing the metric tree on loading parameters was intended to provide a
...
when the loadings themselves are invariant.

Accordingly, the present findings complement the EFA/CFA-tree framework
by highlighting the distinction between detecting measurement
heterogeneity and correctly attributing that heterogeneity to a
particular level of MI. Tree-based procedures provide considerable
flexibility for identifying covariates and regions of the covariate
space associated with parameter instability, but the interpretation of a
split as evidence of specifically metric or scalar MNI require
additional examination of the underlying measurement parameters. This
consideration is especially relevant when tree procedures are
incorporated into sequential MI workflows in which conclusions at one
level determine whether subsequent levels of invariance are evaluated.

Generally speaking, the tree-based approaches may be especially valuable
when researchers wish to investigate several potential sources of MNI
simultaneously or when interactions and nonlinearities cannot readily be
specified in advance. MNLFA, as one contrasting example here, remains
advantageous when specific moderator effects are theoretically motivated
and parameter-level estimates of those effects are of substantive
interest.

More generally, these methods occupy different positions along an
exploratory--confirmatory continuum. Tree-based procedures are well
suited to identifying previously unspecified sources and forms of
measurement heterogeneity, whereas MNLFA is naturally suited to testing
explicitly parameterized moderation hypotheses. This distinction is
consistent with the broader argument that exploratory tree methods
should complement rather than replace confirmatory MI analyses. The
choice of method should therefore follow from the purpose of the
analysis---exploring possible sources and forms of MNI, testing specific
moderation hypotheses, or both---rather than from the expectation that a
single method will be optimal under all circumstances.

\subsection{Limitations and Future
Research}\label{limitations-and-future-research}

Several limitations delimit the conclusions that can be drawn from the
present simulations. First, the simulation intentionally focused on a
restricted set of functional forms comprising linear, quadratic, and
sigmoid moderation. These conditions capture substantively different
forms of smooth and nonlinear change, but they do not exhaust the range
of relationships that may occur in applied research. Other forms,
including asymmetric nonlinear effects, multiple turning points,
discontinuities, different intervals, or more complex interactions among
moderators, may produce different comparative results. The strong
performance of SEM trees across the present functional forms should
therefore not be generalized to arbitrary forms of MNI.

Second, the simulation considered a relatively simple measurement model
with one latent factor and four continuous indicators. This design
allowed the moderator effects and their location in the measurement
model to be manipulated systematically, but applied measurement models
are often substantially more complex. Multifactor models,
cross-loadings, correlated residuals, ordinal indicators, and varying
degrees of baseline model misspecification may all affect the behavior
of MNLFA and SEM trees. In particular, the consequences of parameter
instability outside the focal parameter class observed here suggest that
model complexity and misspecification may be important determinants of
the localization properties of score-based tests.

Third, the present simulations focused on a small number of candidate
moderators and controlled data-generating conditions. Tree-based
procedures are particularly attractive when many possible covariates are
available, but this setting introduces additional questions concerning
correlated candidate variables, variable-selection stability, minimum
node sizes, and multiplicity correction. The performance of SEM trees
when several strongly correlated covariates compete to explain the same
parameter instability therefore warrants further investigation.
Similarly, future work should examine the stability of estimated split
points across repeated samples. A cutoff that is selected frequently but
varies substantially in its numerical location has different substantive
implications from a sharply identified and highly reproducible
threshold.

Fourth, the present comparison necessarily depended on specific
implementations of the two methodological frameworks. Most importantly,
the scalar tests were not structurally identical across MNLFA and SEM
trees. As discussed above, the SEM-tree scalar analysis was estimated
separately, whereas the MNLFA scalar test was embedded in the sequential
comparison of increasingly constrained models. Future methodological
work could investigate alternative testing strategies that make the
target hypotheses more directly comparable across methods. Such work
would help determine to what extent the observed differences in scalar
performance reflect the general properties of parametric versus
tree-based moderation and to what extent they arise from the particular
testing procedures used here.

Finally, the empirical application was intended to illustrate how the
methods characterize MNI in a substantive data set rather than to
establish definitive age or cultural thresholds for the CASP-12. In
particular, data-driven age cutoffs identified by SEM trees should be
interpreted as sample-specific descriptions until their stability has
been evaluated in independent data. Replication across SHARE waves or
other samples would provide a useful opportunity to examine whether the
same partitions and parameter differences recur. More generally,
replication or cross-validation of tree-derived structures is an
important direction for applied work using recursive partitioning to
investigate MI.
